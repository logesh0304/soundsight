...
def getText(objs):
    ...
    #size: area
    for x in objs : x['size']=x['w']*x['h']
    for x in objs : x['mid']=(x['x']+int(x['w']*0.5), x['y']+int(x['h']*0.5))
    objs.sort(key=lambda x: x['size'], reverse=True)

    # norm size sf=scalefactor
    try:
        sf=osr[objs[0]['cls']]/objs[0]['size']
        for x in objs : x['normsize']=x['size']*sf
    except KeyError:
        pass
    ...
    for i in range(1,len(objs)) :
        ...
        #print(objs[i]['mid'])
        # for x 
        if objs[i]['mid'][0]< shrinked_region[0] : objs[i]['region'][0]='left'
        elif objs[i]['mid'][0]> shrinked_region[0] + shrinked_region[2] : objs[i]['region'][0]='right'
        else : objs[i]['region'][0]=''

        # for y
        if objs[i]['mid'][1]<shrinked_region[1] : objs[i]['region'][1]='top'
        elif objs[i]['mid'][1]> shrinked_region[1] + shrinked_region[3] : objs[i]['region'][1]='below'
        else : objs[i]['region'][1]=''

        #for z
        try:
            if objs[i]['normsize'] > osr[objs[i]['cls']] + osr[objs[i]['cls']]*fb_percentage : objs[i]['region'][2]='front'
            elif objs[i]['normsize'] < osr[objs[i]['cls']] - osr[objs[i]['cls']]*fb_percentage: objs[i]['region'][2]='behind'
            else : objs[i]['region'][2]=''
        except KeyError:
            objs[i]['region'][2]=''
